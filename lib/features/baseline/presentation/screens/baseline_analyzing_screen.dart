...
class _BaselineAnalyzingScreenState
  ...
  void initState() {
    ...
    // 다음 마이크로태스크로 미룬다.
    Future.microtask(() {
      if (mounted) ref.read(baselineUploadControllerProvider.notifier).submit();
    });
  }
            ...
            child: SingleChildScrollView(
              ...
              child: uploadState.hasError
                  ? _ErrorContent(
                      ...
                      needsMeasurement:
                          uploadState.error is BaselineMeasurementException,
                      onRemeasure: () {
                        ref
  ...
  String _errorMessage(Object? error) {
    if (error is NetworkException) {
      return '서버에 연결하지 못했어요. 연결을 확인한 뒤 다시 시도해주세요.';
    }
    if (error is ServerException) {
      return '측정 결과를 저장하지 못했어요. 잠시 후 다시 시도해주세요.';
    }
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Gap.h24,
        PrimaryButton(
          label: needsMeasurement ? '다시 측정하기' : '다시 시도하기',
          onPressed: needsMeasurement ? onRemeasure : onRetry,
        ),
      ],
    );
  }
